<template>
    <div class="">
        ...
        <p class="text-gray-500 text-center text-sm">Please enter the email you used to request the password reset.</p>
        <form @submit.prevent="submitForm" class="flex flex-col gap-3">
            ...
            <button type="submit" class="btn w-full my-3">Reset Password</button>
        </form>
    </div>
</template>
...
const submitForm = async () => {
    try {
        await authStore.resetPassword(props.token, email.value, password.value, confirmPassword.value);
        router.push('/auth');
    } catch (error) {
